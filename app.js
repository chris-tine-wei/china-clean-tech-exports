...
// --- technology layers (the toggle) -----------------------------------------
// Solar keeps the existing hand-tuned files and behaviour; the other techs load
// lazily from data/flows-<tech>.json + data/flows-monthly-<tech>.json (same schema).
// ACTIVE is what the *map* draws; FLOWS stays solar so the narrative charts never move.
const TECH_LABEL = { solar: "solar", ev: "EVs", battery: "batteries", wind: "wind tech", grid: "grid tech" };
let ACTIVE = { tech: "solar", countries: [], world: null };  // world: per-month world total ($M) for the scrubber; null => use solar chart series
let MONTHLY_SOLAR = null;                                     // solar monthly, kept so we can switch back
const TECH_CACHE = {};                                       // {tech: {countries, monthly, world}}
const arcVal = (c) => (c.last12 != null ? c.last12 : (c.solar_last12 != null ? c.solar_last12 : 0));
...
Promise.all([
  ...
  .then(([flows, centroids, monthly]) => {
    ...
    ACTIVE.countries = flows.countries || [];   // solar is the default map layer
    if (monthly && monthly.series) {
      ...
      MONTHLY_SOLAR = MONTHLY;
    }
    TECH_CACHE.solar = { countries: ACTIVE.countries, monthly: MONTHLY_SOLAR, world: null };
...
/* --------------------------------------------------- technology toggle */
// The pills swap ONLY the flow-map layer: arc destinations/weights, the timeline
// series and the tooltip/legend wording. The scrollytelling narrative and its three
// charts stay solar (they read FLOWS, which never changes). Non-solar layers load
// lazily and are cached, so no page reload and each layer is fetched at most once.
let SOLAR_LEGEND_HTML = null;

async function loadTech(tech) {
  if (TECH_CACHE[tech]) return TECH_CACHE[tech];
  const [flows, monthly] = await Promise.all([
    fetch(`data/flows-${tech}.json`).then((r) => r.json()),
    fetch(`data/flows-monthly-${tech}.json`).then((r) => r.json()),
  ]);
  let mx = 1;
  for (const k in monthly.series) for (const v of monthly.series[k]) if (v > mx) mx = v;
  const res = {
    countries: flows.countries || [],
    monthly: { months: monthly.months, series: monthly.series, max: mx },
    world: flows.worldMonthly || null,
  };
  TECH_CACHE[tech] = res;
  return res;
}

function updatePills(tech) {
  document.querySelectorAll("#tech-toggle .tech-pill").forEach((b) => {
    const on = b.dataset.tech === tech;
    b.classList.toggle("active", on);
    b.setAttribute("aria-pressed", on ? "true" : "false");
  });
}

// solar keeps its original hand-written legend; other techs get generic wording
// (no GW estimate — that ~$0.10/W convention is solar-only)
function setLegendForTech(tech) {
  const lg = document.getElementById("legend");
  if (!lg) return;
  if (SOLAR_LEGEND_HTML === null) SOLAR_LEGEND_HTML = lg.innerHTML;   // snapshot the solar legend once
  if (tech === "solar") { lg.innerHTML = SOLAR_LEGEND_HTML; return; }
  const label = TECH_LABEL[tech];
  lg.innerHTML =
    '<div class="lg-eyebrow">How to read this</div>' +
    '<div class="lg-title">Each arc is one country’s Chinese ' + label + '</div>' +
    '<div class="bars">' +
      '<span class="bar"><i style="height:8px"></i><b>less</b></span>' +
      '<span class="bar"><i style="height:15px"></i><b></b></span>' +
      '<span class="bar"><i style="height:22px"></i><b></b></span>' +
      '<span class="bar"><i style="height:26px"></i><b>more</b></span>' +
    '</div>' +
    '<div class="lg-note">Thicker, brighter arcs carry more US$ of Chinese ' + label + '. Last 12 months.</div>';
}

async function switchTech(tech) {
  if (!TECH_LABEL[tech] || tech === ACTIVE.tech) return;
  updatePills(tech);   // instant feedback; data may take a beat on first load
  let data;
  try { data = await loadTech(tech); }
  catch (e) { console.error("Could not load tech layer:", tech, e); updatePills(ACTIVE.tech); return; }

  ACTIVE.tech = tech;
  ACTIVE.countries = data.countries;
  ACTIVE.world = data.world;
  MONTHLY = data.monthly;

  // rebuild arcs + destination points for the new set of countries
  ARC_FC = buildArcFeatures();
  if (MAP && MAP.getSource("arcs")) MAP.getSource("arcs").setData(ARC_FC);
  if (MAP && MAP.getSource("pts")) MAP.getSource("pts").setData(buildPointFeatures());

  // re-apply the current view: on the timeline step keep the playhead, otherwise
  // resize every arc by its 12-month value for the active step's region filter
  if (currentStep === 5 && MONTHLY) {
    setTimelineMonth(Math.min(tlMonth, MONTHLY.months.length - 1));
  } else {
    applyWeights(currentStep < 0 ? 0 : currentStep);
  }
  setLegendForTech(tech);
}

function initTechToggle() {
  const wrap = document.getElementById("tech-toggle");
  if (!wrap) return;
  wrap.querySelectorAll(".tech-pill").forEach((b) => {
    b.addEventListener("click", () => switchTech(b.dataset.tech));
  });
}
    ...
    if (tip) {
      MAP.on("mousemove", "pts-hit", (e) => {
        ...
        if (f.origin === true || f.origin === "true") {
          ...
          if (ACTIVE.tech === "solar") {
            // GW estimate (~$0.10/W) is a solar-only convention — keep it exactly as before
            const gw = v / 100;
            tip.innerHTML = "<b>" + f.name + "</b><span>" + dollars + " solar &middot; ~" + (gw >= 1 ? gw.toFixed(1) : gw.toFixed(2)) + " GW (12 mo)</span>";
          } else {
            tip.innerHTML = "<b>" + f.name + "</b><span>" + dollars + " " + TECH_LABEL[ACTIVE.tech] + " (12 mo)</span>";
          }
        }
...
function drawScrubber() {
  ...
  // solar uses its narrative world series; other techs carry their own world total
  const data = ACTIVE.world || (FLOWS.worldByCatMonthly && FLOWS.worldByCatMonthly["Solar PV"]) || [];
